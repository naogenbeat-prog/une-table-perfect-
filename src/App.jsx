...
const detailPages = {
  cocktail: {
    ...
    desc: "華やかなカクテルパーティーやレセプション。彩り豊かなフィンガーフードで会話も弾む特別な空間を演出します。",
    photos:[
  ...
  standing: {
    title: "Standing Party",
    desc: "企業様の懇親会や立食パーティー。ブランドイメージを高める洗練された料理とおもてなしを提供します。",
    photos:[
  ...
  private: {
    ...
    desc: "オーダーメイドで高級レストランに変わる、プライベート空間での特別な贅沢。",
    photos:[
  ...
  gallery: {
    title: "All Gallery",
    desc: "美しき一皿の記録。これまでのケータリングの数々をご覧ください。",
...
const Navbar = ({ isScrolled, currentView, onViewChange }) => {
  ...
  return (
    <nav className={`fixed w-full z-50 transition-all duration-700 ${isScrolled || currentView !== "home" ? "bg-zinc-950/95 backdrop-blur-md py-3 md:py-4 border-b border-zinc-800" : "bg-transparent py-4 md:py-6"}`}>
      <div className="w-full px-4 md:px-12 flex flex-col md:flex-row justify-start items-center gap-2 md:gap-16">
        <img onClick={() => onViewChange("home")} src={CONFIG.logoImage} alt={CONFIG.brandName} className="h-8 md:h-12 cursor-pointer object-contain" />
...
const Services = ({ onViewChange }) => (
  <section id="services" className="py-24 bg-zinc-900 px-6">
    <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-12">
      {[
        { id: "cocktail", title: "Cocktail", img: images.serviceWedding, desc: "華やかなレセプションを、", highlight: "彩り豊かなフィンガーフードで。" },
        { id: "standing", title: "Standing", img: images.serviceCorporate, desc: "企業様の懇親会を彩る、", highlight: "洗練された立食スタイル。" },
        { id: "private", title: "Private", img: images.servicePrivate, desc: "高級レストランに変わる、", highlight: "オーダーメイドの特別な贅沢。" },
      ].map((s, i) => (
...
const Gallery = ({ onViewChange }) => (
  <section id="gallery" className="py-24 md:py-32 px-4 md:px-8 max-w-screen-2xl mx-auto">
    ...
    <div className="text-center mt-16">
      <button onClick={() => onViewChange("gallery")} className="border border-amber-500/50 text-amber-500 px-10 py-4 hover:bg-amber-600 hover:text-white transition-all text-xs tracking-[0.2em] uppercase">
        すべての写真を見る
      </button>
    </div>
  </section>
...
const DetailView = ({ viewId, onViewChange }) => {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 pt-32 pb-24 px-6 md:px-12 animate-[fadeIn_0.5s_ease-out]">
      <div className="max-w-7xl mx-auto">
        <button onClick={() => onViewChange("home")} className="flex items-center text-amber-500 hover:text-white transition-colors mb-12 tracking-widest text-sm uppercase">
          <ChevronLeft size={16} className="mr-2" /> Top Page
        </button>
        <div className="mb-16">
          <h2 className="font-serif text-3xl md:text-5xl text-white font-light mb-6">{data.title}</h2>
    ...
    <div className="min-h-screen bg-zinc-950 text-stone-300 font-sans selection:bg-amber-900 selection:text-white">
      ...
      {currentView === "home" ? (
        <>
          ...
          <MenuSection />
          <Testimonials />
          <Gallery onViewChange={setCurrentView} />
          <Contact />
        </>
      ) : (
        <DetailView viewId={currentView} onViewChange={setCurrentView} />
      )}
