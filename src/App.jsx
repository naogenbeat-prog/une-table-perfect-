...
const images = {
  concept1: "/business-16.png", // ★ご指定通り戻しました
...
const Hero = () => (
  <section className="relative h-screen overflow-hidden">
    ...
    <div className="relative z-10 flex flex-col h-full px-4 max-w-4xl mx-auto pt-32 md:pt-40 text-center">
      ...
      <div className="pb-32 text-stone-300">
  ...
  <section id="concept" className="py-24 md:py-40 px-6 max-w-7xl mx-auto flex flex-col md:flex-row items-center gap-20">
    ...
    <div className="w-full md:w-1/2 text-stone-300">
      <div className="flex items-center gap-4 mb-8">
        ...
        <h3 className="text-white tracking-[0.2em] text-sm uppercase">CONCEPT</h3>
      </div>
        ...
        <div key={i} onClick={() => onViewChange(s.id)} className="group cursor-pointer">
          ...
          <div className="flex items-start">
            ...
            <div>
              ...
              {/* 文字サイズを大きく調整 (text-lg) */}
              <p className="text-stone-300 text-lg tracking-widest leading-relaxed">
                {s.desc}<br />
                <span className="text-amber-500 font-bold block mt-1">{s.highlight}</span>
              </p>
            </div>
          </div>
        </div>
...
const Testimonials = () => (
  <section className="py-24 bg-zinc-900/50 px-6 border-y border-zinc-800">
  ...
  <section id="gallery" className="py-24 px-4 max-w-screen-2xl mx-auto bg-zinc-950">
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      {images.gallery.map((img, idx) => (
        <div key={idx} className="relative overflow-hidden group aspect-square bg-zinc-900">
          <img src={img} className="w-full h-full object-cover transition-transform duration-[2000ms] hover:scale-110 opacity-90 hover:opacity-100" alt={`Gallery ${idx}`} />
        </div>
      ))}
    </div>
  </section>
        ...
        <div className="bg-zinc-900/50 p-8 md:p-14 border border-zinc-800">
          {status === "success" ? (
            ...
            <form className="space-y-8" onSubmit={handleSubmit}>
              ...
              <textarea name="message" value={formData.message} onChange={(e)=>setFormData({...formData, message:e.target.value})} required placeholder="ご相談内容" className="w-full bg-transparent border-b border-zinc-700 text-white py-2 outline-none focus:border-amber-500 resize-none h-32"></textarea>
...
const DetailGalleryView = ({ viewId, onBack }) => {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 pt-28 pb-24 px-6 animate-[fadeIn_0.5s_ease-out]">
      <div className="max-w-7xl mx-auto">
        <button onClick={onBack} className="text-amber-500 hover:text-white mb-12 flex items-center tracking-widest text-xs uppercase"><ChevronLeft size={16} className="mr-2" /> Back to Top</button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{data.photos.map((img, i) => (<img key={i} src={img} className="w-full h-80 object-cover" />))}</div>
      </div>
    </div>
  );
};
